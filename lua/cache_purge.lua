...
function list_files(cache_path, purge_pattern)
        local result = exec("/usr/bin/find " .. cache_path .. " -type f | /usr/bin/xargs --no-run-if-empty -n1000 /bin/grep -El -m 1 '".. purge_pattern .. "' | grep -v 'No such' 2>&1")
        if result == "" then
                return {}
        end
        return explode("\n", result)
...
function purge_multi()
        local files = list_files(ngx.var.lua_purge_path, ngx.var.request_uri)
...
if ngx ~= nil then
        ...
        local count = purge_multi(ngx.var.request_uri)
